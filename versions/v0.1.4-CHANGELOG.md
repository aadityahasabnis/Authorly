...
**Impact**: 🔴 **CRITICAL** - Core feature was completely broken
...
**Impact**: 🟢 **LOW** - Visual consistency fix
...
**Solutions Applied**:
  - Changed URL `line-height: 1.5` → `1`
  - Added explicit `height: 24px` to URL element
  - Changed buttons from `30px × 30px` → `24px × 24px`
  - Changed divider from fixed `height: 20px` → `align-self: stretch`
  - Adjusted icon size to `14px × 14px`
  - Changed URL `display: block` → `display: flex` with `align-items: center`
...
4. **tsconfig.node.json** (new file)
   - Created for build tool type checking
   - Extends main tsconfig.json
   - Includes vite.config.ts

5. **vite.config.ts**
   - Added `minify: 'esbuild'` for production builds
   - Fixed path imports to use proper ES modules syntax
   - Added `__dirname` equivalent for ES modules

6. **package.json**
   - Added `@types/node: ^25.2.0` to devDependencies

### New Files Created

1. **versions/v0.1.4-CHANGELOG.md** - This file
2. **versions/CODE_REVIEW_FINDINGS.md** - Detailed code review
3. **versions/UPGRADE_GUIDE.md** - Developer upgrade instructions
4. **versions/PROJECT_STATUS.md** - Project status summary
5. **tsconfig.node.json** - TypeScript config for build tools
...
✅ **Link Creation**
- [x] Select text and click link button → popup appears
- [x] Enter URL and press Enter → link created successfully
- [x] Link maintains text formatting (bold, italic, etc.)
- [x] Cursor positioning after link creation is correct

✅ **Link Preview Card**
- [x] Hover over link → preview appears after 300ms
- [x] Move mouse away → preview closes after 200ms
- [x] Move from link to preview → preview stays open
- [x] Move away from preview → preview closes
- [x] Click Edit button → opens link editor with current URL
- [x] Click Remove button → removes link, keeps text
- [x] Click URL → opens in new tab
- [x] Scroll page → preview closes immediately
...
✅ **Edge Cases**
- [x] Empty selection → link button disabled
- [x] Clicking outside popup → popup closes
- [x] Multiple rapid hovers → preview doesn't glitch
- [x] Preview near viewport edge → stays visible
...
```
dist/style.css      41.35 kB │ gzip:  6.54 kB  (+0.01 kB)
dist/index.esm.js  208.58 kB │ gzip: 45.27 kB  (+0.02 kB)
dist/index.cjs.js  162.92 kB │ gzip: 40.49 kB  (unchanged)
```
